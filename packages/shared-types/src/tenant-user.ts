import { z } from "zod";
import { ROLES, PALETTE_COLOR_TOKENS, tenantRolesSchema } from "./enums";

export const createTenantUserSchema = z.object({
  email: z.string().email(),
  password: z.string().min(8, "Senha deve ter ao menos 8 caracteres"),
  name: z.string().min(1),
  roles: tenantRolesSchema,
  // Opcional — relevante só pra quem tem DENTIST (cor na sidebar da Agenda);
  // se omitido, o serviço atribui a próxima cor da paleta por índice.
...
export const updateTenantUserSchema = z.object({
  roles: tenantRolesSchema.optional(),
